...
- **Third-party dependencies refreshed.** `Cargo.lock` takes 100 crates to their
...
- A gate between building and publishing in `release.yml`. Every publish job now
  waits on it, and it opens only for a pushed `refs/tags/v*` whose commit has a
  successful `ci.yml` run and nothing red. Previously four publish chains fired
  in parallel and `cargo-publish` waited on nothing at all, so crates.io could
  take a version while the wheels were still compiling. `wasm-publish` was split
  into `wasm-build` and `wasm-publish` so its build sits before the gate and npm
  receives the same tarball the release page attaches.
- `go-mirror` builds and tests the assembled module before pushing it. The
  mirror repository has no CI, so a wrongly assembled tree previously went
  public and surfaced when someone else's `go get` broke.
- Build provenance now covers the NuGet package, the Java jar and the six C ABI
  archives, not only the crates, wheels and sdist.
- CodeQL analyses the C#, Java, Go and C/C++ surfaces, with the config file that
  excludes the napi-generated FFI glue. Example code is deliberately included.
- `actionlint.yml` lints the workflows themselves -- shell errors in `run:`,
  unknown keys, undefined action inputs -- with a checksum-pinned binary.
- `codspeed.yml` measures the benches under instrumentation and reports
  regressions on the pull request. The bench crate aliases `criterion` to
  `codspeed-criterion-compat`, without which the run reports nothing.
- CI job `semver`: `cargo-semver-checks` against the last published release of
  each of the four published crates.
- CI job `python-wheel-container-smoke`: builds the manylinux and musllinux
  wheels and imports them, the musl one inside a musl container. That path was
  previously first exercised at the tag.
- `scripts/check_example_specs.py` holds `schema/strategy_spec.schema.json`
  against the seven shipped strategy specs. The Rust test proved they parse;
  nothing proved the published schema still described the same DSL.
- `examples/c/golden_test.c` asserts golden parity through the C ABI, the
  surface all nine other bindings go through and the only one that had no golden
  test of its own. The case list is generated by CMake from the fixture
  directory rather than hand-kept.
- `osv-scanner` runs in CI beside `cargo-deny`, covering the npm, maven, nuget
  and go lockfiles that `cargo-deny` cannot see.
...
- Branch protection is enabled on `main`: linear history, no force-push, no
  deletion.
- Action pins raised to the family version: `codeql-action` v4.37.9,
  `setup-java` v6.0.0, `r-lib/actions/setup-r` v2.13.0, `action-gh-release`
  v3.0.3, `taiki-e/install-action` v2.87.1. Each SHA is the commit the tag points
  at rather than the tag object.
