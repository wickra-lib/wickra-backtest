...
CLASS_STREAMING = {
    ...
    # Same members as Node, spelled the same way: both are consumed from
    # JavaScript. `stream_free` maps to `close`, since wasm-bindgen already owns
    # the name `free` for reclaiming the instance itself.
    "wasm": (
        NODE_MEMBERS,
        lambda n: re.sub(r"_(\w)", lambda m: m.group(1).upper(), n),
        r"@NAME@\s*=|pub fn @NAME@\s*\(|js_name = @NAME@",
        r"pub struct StreamingBacktest\b",
    ),
    "node": (
        NODE_MEMBERS,
        lambda n: re.sub(r"_(\w)", lambda m: m.group(1).upper(), n),
...
def main() -> int:
    ...
    # the direction that breaks callers. Recognised names are the contract itself
    # plus the members a class-shaped binding spells it with, so a name shows up
    # here only when it is genuinely a reach no export backs.
    wasm = read(BINDINGS["wasm"][0])
    recognised = set(contract) | set(CLASS_STREAMING["wasm"][0].values()) | {
        "new",       # the constructor
        "free",      # generated by wasm-bindgen to reclaim the instance
        "is_finished",  # the same convenience getter the other object bindings carry
    }
    ahead = [m for m in re.findall(r"pub fn ([a-z_0-9]+)", wasm) if m not in recognised]
    if ahead:
